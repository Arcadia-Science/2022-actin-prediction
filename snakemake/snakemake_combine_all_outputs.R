...
#blast_results <- read_tsv("outputs/blast/blastp_results_empty_rm.out")

# this is a hand-curated file that uses the uniprot protein names to inform what class each actin sequence falls into
class_map <- read_csv(snakemake@input[['class_map']]) %>%
#class_map <- read_tsv("inputs/uniprot_gene_name_to_class_map.tsv") %>%
  filter(!is.na(class))

# read in average percent identity results
avg_pid <- read_tsv(snakemake@input[['avg_pid']]) %>%
#avg_pid <- read_tsv("outputs/mean_pid/all_avg_pid.tsv") %>%
  distinct() %>%
...
#features <- read_tsv("outputs/shared_feature_residues/3_shared_residue_summaries_combined/all_shared_residues_combined.tsv") %>%
  distinct() %>%
  filter(protein %in% blast_results$sseqid)

# read in the hmm results
hmm <- read_tsv(snakemake@input[['all_hmm']]) %>%
#hmm <- read_tsv("outputs/hmm/hmmscan/all-hmmscan-tbl-out.tsv") %>%
  distinct() %>%
...
join3 <- full_join(join2, class_map, by = c("protein_names" = "gene_name"))
